...
const SongStructuredData = ({ songs }: { songs: any[] }) => {
  return (
    <>
      {songs.map((song, index) => {
        ...
        const structuredData = {
          ...
          "name": song.title,
          "url": `https://elkanisa.com/songs/${songSlug}`,
        ...
        return (
          <Script
            key={`song-structured-data-${index}`}
            id={`song-structured-data-${index}`}
...
const HiddenSongsLinks = ({ songs }: { songs: any[] }) => {
  return (
...
// المكون الرئيسي
export default function SEOSongsComponent() {
  ...
  return (
    <>
      ...
      <SongStructuredData songs={songs} />
      <HiddenSongsLinks songs={songs} />
    </>
  );
}
